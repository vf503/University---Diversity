...
$.extend({
    webox: function(option) {
        ...
        $(document).ready(function(e) {
            $('.webox').remove();
            $('.background').remove();
            var width = option['width'] ? option['width'] : 310;
            var height = option['height'] ? option['height'] : 250;
            $('.FullContent').append('<div class="background" style="display:none;"></div><div class="webox" style="width:' + width + 'px;height:' + height + 'px;display:none;"><div id="inside" style="height:' + height + 'px;"><h1 id="locked" onselectstart="return false;">' + (option['title'] ? option['title'] : '') + '<a class="span" href="javascript:location.href=window.location.href;"></a></h1>' + (option['iframe'] ? '<iframe class="w_iframe" src="' + option['iframe'] + '" frameborder="0" width="100%" scrolling="yes" style="border:none;overflow-x:hidden;height:' + parseInt(height - 30) + 'px;"></iframe>' : option['html'] ? option['html'] : '') + '</div></div>');
            //.net控件在表单内部才生效
            $(".LoginInput").attr("value", ""); //
            //var LoginDivHtml = $("#PopupBox")[0].innerHTML;
            $("#PopupBox").remove();
            //删除源
